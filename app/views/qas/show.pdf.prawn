...
teams = [["Team Name", "Completed Items"]]
teams += @teams.map do |team|
  [
  team.name,
  team.qas.count > 0 ? ((team.qas.where(:completed => true).where("qas.created_at >= ?", 2.weeks.ago.utc).count / team.qas.count.to_f) * 100).to_i : 0
  ]
end

table(teams, :header => true,
             :column_widths => [440, 100],
